...
router.post("/sendmail", morgan(':method :url :status :res[content-length] - :response-time ms'), allcalling.mail);
router.get("getlink/aws",allcalling.getDetails)
router.post(
  "/signup/authcheck", 
  [
...
router.post(
  "/signin/authcheck", morgan("combined"),
  [
    body("email").isEmail().withMessage("Please enter a valid email address"),
    body("password").notEmpty().withMessage("Password is required"),
  ],
